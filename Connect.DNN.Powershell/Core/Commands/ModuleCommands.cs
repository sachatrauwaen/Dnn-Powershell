...
namespace Connect.DNN.Powershell.Core.Commands
{
    public class ModuleCommands
    {
        ...
        public static string ApiModule(Data.Site site, int pageId, int moduleId, string moduleName, string controller, string action, string httpMethod, string payload)
        {
            var response = DnnPromptController.ProcessApi(site, 5, pageId, moduleId, moduleName, controller, action, httpMethod, payload);
            //var result = Newtonsoft.Json.JsonConvert.DeserializeObject<ConsoleResultModel<ModuleInfoModel>>(response.Contents);
            //result.AssertValidConsoleResponse();
